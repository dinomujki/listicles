...
def List():
    ...
    html=urllib.urlopen(prompt).read()
    soup=BeautifulSoup(html)
    l = soup.find_all('table', class_='wikitable sortable')[0]
    # print l
    ...
    for item in rawitems:
        if counter == 10:
            break
        if len(item.find_all('td')) != 0 and len(item.find_all('a')) > 0:
            ...
            googleurl = "https://www.google.com/search?q=wikipedia+"+str(googlenames)
